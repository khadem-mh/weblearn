...
export default function CourseInfo() {
  ...
  const getCourseDetails = () => {
    ...
    fetch(`https://kind-tips-jam.loca.lt/v1/courses/${params.course}`, {
      method: 'GET',
      headers: {
    ...
    if (JSON.parse(localStorage.getItem("user")) && JSON.parse(localStorage.getItem("user")).token) {
      if (course.price === 0) {
        swal({
          ...
          if (result) {
            fetch(`https://kind-tips-jam.loca.lt/v1/courses/${course._id}/register`, {
              method: "POST",
              headers: {
          ...
          if (result) {
            swal({
              ...
              if (code === null) {
                ...
                fetch(`https://kind-tips-jam.loca.lt/v1/offs/${code}`, {
                  method: "POST",
                  headers: {
                    Authorization: `Bearer ${JSON.parse(localStorage.getItem("user")).token
                      }`,
                  ...
                  .then((code) => {
                    fetch(
                      `https://kind-tips-jam.loca.lt/v1/courses/${course._id}/register`,
                      {
                        method: "POST",
                        headers: {
                          Authorization: `Bearer ${JSON.parse(localStorage.getItem("user")).token
      ...
      swal({
        title: 'لطفا ابتدا در وبسایت لاگین کنید',
        icon: 'warning',
        buttons: 'باشه'
      }).then(() => {
        navigate('/login')
