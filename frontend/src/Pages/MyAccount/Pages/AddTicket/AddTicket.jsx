...
export default function AddTicket() {
  ...
  useEffect(() => {
    fetch(`https://kind-tips-jam.loca.lt/v1/tickets/departments`)
      .then(res => res.json())
  ...
  useEffect(() => {
    if (departmentID && departmentID !== -1) {
      fetch(`https://kind-tips-jam.loca.lt/v1/tickets/departments-subs/${departmentID}`)
        .then(res => res.json())
  ...
  const isTypeHasSupport = val => {
    if (val === "63b688c5516a30a651e98156") {
      fetch(`https://kind-tips-jam.loca.lt/v1/users/courses`, {
        method: "GET",
        headers: {
  ...
  const sendNewTicket = e => {
    ...
    if (
      ...
      fetch(`https://kind-tips-jam.loca.lt/v1/tickets`, {
        method: "POST",
        headers: {
          'Authorization': `Bearer ${JSON.parse(localStorage.getItem('user')).token}`,
